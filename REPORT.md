...
Структурированность данных: Реляционная модель предоставляет структуру для организации данных в виде таблиц, столбцов и строк. Это позволяет легко понимать и управлять данными.

Целостность данных: Реляционная модель поддерживает ограничения целостности, такие как уникальность значений, ссылочная целостность и ограничения целостности на уровне таблиц. Это помогает поддерживать консистентность данных.

Простота использования: Реляционные базы данных обладают простым и интуитивно понятным языком запросов - SQL. Это делает их доступными для широкого круга пользователей и разработчиков.

Гибкость: Реляционная модель позволяет выполнять сложные запросы, объединять данные из разных таблиц и создавать связи между ними. Это обеспечивает гибкость в анализе данных и получении нужной информации.

Недостатки реляционного представления:

Сложность масштабирования: Реляционные базы данных могут столкнуться с проблемами масштабирования при обработке больших объемов данных или при высокой нагрузке. Это связано с необходимостью поддержки целостности данных и выполнения сложных операций объединения.

Жесткая схема данных: Реляционная модель требует определения схемы данных заранее, что может быть неудобно в случае изменения требований или добавления новых типов данных. Это может потребовать изменения существующей схемы и перестроения базы данных.

Ограничения производительности: Некоторые операции, такие как объединение таблиц или выполнение сложных запросов, могут быть затратными с точки зрения производительности. Это может быть особенно заметно при работе с большими объемами данных.

Опишите принцип реализации всех предикатов, осуществляющих запросы к данным.

## Выводы

Данная лабораторная работа научила меня мыслить структурировано, объединяя все полученные знания и применяя их на практике, благодаря очень удобному формоту данной лабораторной работы я смог подчеркнуть для себя много нового, обобщил правила применения предикатов и написал свои собственные для работы с реляционным представлением данных, в результате смог качественно реализовать задуманные идеи.
